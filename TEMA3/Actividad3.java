...
public class Actividad3 {
    ...
    public static void game () {
        ...
        while (!winner) {
            ...
            fly = changePositionFly(selection, array, fly);
            if (fly == selection) {
                winner = true;
            }
        }
    }
    public static int whereIsFly(int[]array) {
        return array[random.nextInt(14)];
    }
    public static int changePositionFly (int selection, int[]array, int fly) {
        if (selection == (fly-1) || selection == (fly+1)) {
            System.out.println("Casi pero no!");
            fly = whereIsFly(array);
